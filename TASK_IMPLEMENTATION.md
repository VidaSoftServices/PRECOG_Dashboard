...
| `/monitoring` | LiveMonitoringPage | Live per-Sensor readings | §"Live Monitoring" via `CLAUDE.md` (rewritten this checkpoint - see decision log #26) | `Sensors_GetSensors`, `*_GetMeasuredTrailingPeriods` | Both | Yes | loading, empty, error, stale, rate-limited | all | n/a (read-only) | 3s round-robin cycle (`useLiveMonitoringSchedule.ts`), one Sensor/tick, pauses on hidden tab/paused/no-Device/route-unmount | Implemented | **Yes (mocked)** |
...
| `telemetry.trailing` (Live Monitoring) | deviceId+sensorId defined, non-curve family | **passive only** - `useTelemetryTrailingPeriods` never fetches itself (`enabled:false`); `useLiveMonitoringSchedule.ts`'s orchestrator drives one `fetchAndCacheTelemetryTrailingPeriods` call per Sensor per 3s tick (`liveMonitoringCycle`), one Sensor at a time, round-robin | orchestrator effect exits entirely (no tick scheduled) while paused/hidden/no-Device/unmounted - not just an interval pause | **0** (explicit per-call override - see decision log #26; the 15s global default silently served stale cache on rotation restart otherwise) | n/a (time-window query) | `signal` forwarded (orchestrator's own `AbortController`, aborted on cleanup) | paused/hidden/no-Device/route-unmount; 429 additionally pauses the whole schedule until the shared cooldown clears |
...
| 26 | Live Monitoring request-storm fix (25+ Sensors triggering a "Too many requests" toast storm) | Single round-robin orchestrator (`useLiveMonitoringSchedule.ts`) replacing 25 independent per-Sensor `refetchInterval`s; a self-rescheduling `setTimeout` loop fetching exactly one Sensor per 3s tick; centralized shared 429/cooldown state; per-call `retry:false`+`staleTime:0` override on the orchestrator's `fetchQuery` call | **Root cause, confirmed from source, not assumed**: (1) each `SensorLiveCard` ran its own `refetchInterval` at the old 15s `liveCompactChart` constant, so N Sensors meant N independent, staggered, uncoordinated polling loops - already wrong at any N; (2) `queryKeys.telemetryTrailing`'s key embedded a fresh `endDate` on every call, so no two fetches for the "same" Sensor ever shared a cache entry, defeating dedup entirely; (3) confirmed via a dedicated backend audit (read-only, `C:\VS\API`) that `MeasuredTrailingPeriods` carries `[EnableRateLimiting("LargeTelemetryRead")]` - a **30 requests/60s per-client-IP** fixed-window limiter, `QueueLimit:0`, genuine `Retry-After` on rejection (`Program.cs`, `appsettings.json`'s `RateLimiting` block) - 25 Sensors × even a modest per-Sensor interval blew this budget immediately, and every browser tab on the same IP shares it. **Two further live bugs found only via real mocked-browser verification** (invisible to fake-timer unit tests, which mock the fetch function away entirely): `queryClient.ts`'s sensible global defaults (retry a 429 once internally; 15s `staleTime`) silently applied to the orchestrator's own `queryClient.fetchQuery()` call too - the global retry issued an uncoordinated second real request for the same Sensor moments after its first 429 (defeating the shared cooldown, observed as 4 real 429s where the design predicts ~1); the global `staleTime` silently served cached data with zero network traffic whenever the rotation restarted (pause/resume, route away/back) within 15s of a Sensor's last fetch (observed as "0 requests on resume" / "rotation stuck after returning to the page"). Both fixed by passing explicit `retry:false`/`staleTime:0` on that one call site only - every other `useQuery` in the app keeps the global defaults. **Confirmed NOT the cause**: React StrictMode's dev-mode double-effect-invocation (a `console.log`-instrumented diagnostic run showed exactly one effect instance, one `instanceTag`, throughout - the cleanup/`AbortController` logic was already correct). **The oft-cited "1024" value is unrelated to any of this** - confirmed via source (`DB_Partitions.txt`) to be PostgreSQL's `max_locks_per_transaction`, a lock-table sizing knob, with no relationship to the HTTP rate limiter or to Npgsql's `MaxPoolSize:40` connection pool (itself confirmed architecturally independent of the rate limiter via an explicit `Program.cs` comment). New sustained load: 25 Sensors × 1 request/tick × 20 ticks/min = **20 req/min, 67% of the 30/60s ceiling, 33% margin** - down from an unbounded/uncoordinated pattern that could burst past the limit on load alone. No API/database file was read-write touched; no rate limit was raised. | `src/lib/pollIntervals.ts`, `src/api/queryKeys.ts`, `src/api/hooks/telemetry.ts`, `src/pages/monitoring/useLiveMonitoringSchedule.ts` (new), `src/pages/monitoring/useLiveMonitoringSchedule.test.tsx` (new, 17 tests), `src/api/hooks/telemetry.test.ts` (+2 regression tests against the real `queryClient` singleton), `src/components/charts/TelemetryChart.tsx` (unrelated axe fix bundled in - see below), `src/pages/monitoring/LiveMonitoringPage.tsx` | Approved (explicit task) |
| 27 | `TelemetryChart`'s `role="img"` canvas had no accessible name (axe `role-img-alt`, found auditing Live Monitoring this checkpoint) | `aria-labelledby` wired from the `<Line>`/`<Chart>` component (via `react-chartjs-2`'s pass-through `canvasProps`) to the existing generated text-summary `<Text>` element's `id` (`useId()`) | `react-chartjs-2`'s `ChartComponent` hardcodes `role: "img"` on every canvas it renders (confirmed in its installed source, `node_modules/react-chartjs-2/dist/index.js`) with no accompanying name unless the caller supplies one via a pass-through prop - true for every chart in the app (Live Monitoring, Smart Analytics, comparison, Issue detail), not something this checkpoint's changes introduced, but a real, fixable gap surfaced by auditing this page as this task required. Reuses the chart's own existing "generated text summary" (already documented in `CLAUDE.md`'s "Visualization components" as the intended screen-reader-facing description) rather than inventing new text, matching the `DateTimeField` accessible-name fix's precedent (Fluent's render-prop `id`/`aria-labelledby` wiring, not a fabricated label) | `src/components/charts/TelemetryChart.tsx` | Approved (accessibility fix within an explicitly-required axe audit) |
...
**Current totals as of the Live Monitoring request-storm fix checkpoint**
(decision log #26/#27 above have the full root-cause detail): **114/114
unit tests passing, 17 files** (up from 95/16 - adds
`useLiveMonitoringSchedule.test.tsx`'s 17 fake-timer tests for the new
orchestrator's timing/rotation/cooldown/cancellation logic, plus 2 new
`telemetry.test.ts` regression tests run against the real production
`queryClient` singleton, deliberately not a fresh test-only one, so they
fail if the `retry`/`staleTime` per-call overrides are ever dropped);
lint/typecheck/production build all still green. A dedicated 5-scenario
mocked-browser Playwright pass (25 checks; real Chromium, real app, only
`http://localhost:5065/**` intercepted) verified: a 25-Sensor page load
fires 1 request, not a burst; each 3s cycle advances by exactly 1 request;
~12.5s of continuous operation stays at an estimated 24 req/min, under the
confirmed 30/60s ceiling; rotation visits distinct Sensor ids; a real
mocked 429 shows the shared `RateLimitBanner` and exactly one toast (no
storm), fires zero requests during the `Retry-After` cooldown, and resumes
with exactly one request after it expires; pausing stops all requests and
resuming fires exactly one; a hidden tab fires none; navigating away stops
the schedule with no leaked timer and navigating back resumes it; zero
horizontal overflow at 7 viewports (1920×1080 down to 360×640); axe found
zero serious/critical violations (the pre-existing `role-img-alt` gap in
`TelemetryChart`'s underlying `react-chartjs-2` canvas, present on every
chart page, was fixed as part of this pass - see decision #27); and zero
unexpected console/page errors (the mock's own intentionally-permanent 429
resource-load messages, needed to exercise the cooldown path, are excluded
from that count on purpose, not by silently swallowing errors). **A genuine
live-API pass (this task's own required final verification step) remains
blocked**: the local API's Docker containers (`VidaSoft.API`, `MLTraining`)
are not running in this environment (only `PGSQL` is up; `VSSQL` is
stopped) and no production credentials are available - starting the API
containers was judged outside the read-only-inspection authorization this
session was given, so it was not attempted. This is the one remaining
concrete blocker for closing out this task end-to-end.
